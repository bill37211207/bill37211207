...
import {GetStepPauseFlag3,GetStepOneFlag3,GetRecipeEnd3,ClearRecipeEnd3} from './processing';
import {UpdateChamberTableCCChart} from './processing';
import './App.css';
var zero = 1;
...
const AppCC = () => {  
  ...
  //        if(true)
  //        {
  //           //delayBetweenPoints = 180;//delayBetweenPoints + 30; if delayBetweenPoints = 180 and count data length = 100 total = 180* 100 =18000 ms
  //           console.log(delayBetweenPoints);
  //           initiChardData(50,55,75,885,95,-105,100);
  //           initi(lablesCount); 
  //           updateRFFinally()
  ...
  const onButtonClick = () => {
    if(!FirstTime)
    {
      ...
      updateRFFinally2();
       ...
       document.getElementById("UpdateBtn3").innerHTML = 'idle';
       FirstTime = true;
    }
...
const onButtonClick3 = () => {
  ...
  const UpdateByExternalAA = ()=> {
    FlagStepPause1 = GetStepPauseFlag3(); 
    FlagStepOneStart = GetStepOneFlag3();
    FlagRecipeEnd = GetRecipeEnd3();
    scan = !FlagRecipeEnd;   
    FlagPrerun =!FlagStepPause1;
      
    if(FlagPrerun || FlagStepOneStart)
    {
      document.getElementById("UpdateBtn3").style.backgroundColor='green';
      document.getElementById("UpdateBtn3").style.color='white';
      document.getElementById("UpdateBtn3").innerHTML = 'Running...';
      if( !RepeatRunning )
      {  
    ...
    {
      document.getElementById("UpdateBtn3").style.backgroundColor='white';
      document.getElementById("UpdateBtn3").style.color='black';
      document.getElementById("UpdateBtn3").innerHTML = 'idle'; 
      RepeatRunning = false;
    }  
   }; 
  ...
  const UpdateCC = ()=>{
    document.getElementById('h1CHC').innerHTML =  'Chamber C_' + PortID;
    var ObjectTemp = UpdateChamberTableCCChart();
    var VararrHeaderstr =[];
    //RFSetPoint =0;
    //RFActualValue =0;
        ...
        Object.entries(ObjectTemp).forEach(([key, value]) => {        
          ...
          if(key==='RecipeTotalProcessTime') 
          {
            ...
            if(TotalTimeProcessParavalueBB === 0)
            {
              ...
              TotalTimeProcessParavalueBB = 0;   
              //console.log('get lablesCount'+ lablesCount);
              Xindex = 0;
 ...
 useEffect(() => {
  ...
  return (
    <div id ='CHC' className="wrapper223Chart2">
       <h1 id = 'h1CHC' >Chamber C_ {PortID}</h1>      
      <div>
      <button id = 'UpdateBtn3' onClick={onButtonClick5}>Update!</button> 
        <Line id = 'line1' data={DataRF} options={options00}  />   
        <Line id = 'line2'data={dataPressure} options={options00} />      
      </div>
    </div>
  );
};
